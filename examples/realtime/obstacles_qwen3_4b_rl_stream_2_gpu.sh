...
# RL on the STREAMING real-time Frogger env (real-time/environment/realtime_frogger,
# env tag `realtime_frogger_stream`). The world runs on a token cadence: every
# tokens_per_movement (=600) tokens the model produces is one movement window == one
# car tick. While the model is still thinking, the current grid is force-fed into its
# reasoning as masked <observation> blocks; when it emits a move, the move is applied
# first, then the rest of the window elapses (cars advance one more cell) and the
# resulting grid is returned. All injected tokens are masked out of the loss.
#
# The only differences from obstacles_qwen3_4b_rl_2_gpu.sh are:
#   * --custom-generate-function-path -> generate_with_obstacles.generate_streaming
#   * the train/eval datasets are built with --env realtime_frogger_stream
#   * the wandb group.
