...
#   * env tag / datasets -> realtime_frogger_stream_200 (tokens_per_movement=2400)
#   * --num-rollout 300
#   * RESUMABLE: fixed run dir (.cache/frogger200-stream); when a checkpoint tracker
...
#     slime/examples/realtime/frogger200_qwen3_4b_rl_stream_2_gpu.sh
...
RUN_DIR="${REPO_ROOT}/.cache/frogger200-stream"
mkdir -p "${RUN_DIR}"
...
#       --env realtime_frogger_stream_200 --train-size 20000 --seed 1234 \
#       --out $HOME/obstacles-seeds/train_realtime_frogger_stream_200.jsonl
#   PYTHONPATH=./real-time python3 slime/examples/realtime/obstacles_data_preprocess.py \
#       --env realtime_frogger_stream_200 --train-size 256 --seed 999 \
#       --out $HOME/obstacles-seeds/eval_realtime_frogger_stream_200.jsonl
ROLLOUT_ARGS=(
   # Each row is {"prompt": <streaming game rules>, "seed": <int>,
   # "metadata": {"env": "realtime_frogger_stream_200"}}; the seed arrives on
   # sample.label and the env is reconstructed from it at rollout time.
   --prompt-data ${ARTIFACT_ROOT}/obstacles-seeds/train_realtime_frogger_stream_200.jsonl
   --input-key prompt
   --label-key seed
   --reward-key score
   --num-rollout 300
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-4B-realtime-frogger-stream-200
   #--wandb-key ${WANDB_KEY}
)
